...
export const renderSurvey = ({ config, core, recs, preview = 5, repo = "", survey = "" }) => {
  ...
  const recsSections = (rows) => {
    ...
    for (const label of windows) {
      ...
      const batch = inWindow.find((r) => r.batchPrUrl);
      const bulk = batch
        ? `<sub>**[Review all ${inWindow.length} in one pull request →](${batch.batchPrUrl})** ` +
          `Delete the lines you do not want, then merge.</sub>`
        : null;
    ...
    if (rest.length) {
      const batch = rest.find((r) => r.batchPrUrl);
      out.push(
        "### Most connected, any year",
        "",
        ...(batch
          ? [
              `<sub>**[Review all ${rest.length} in one pull request →](${batch.batchPrUrl})** ` +
                `Delete the lines you do not want, then merge.</sub>`,
              "",
            ]
          : []),
