  ...
  const recsSections = (rows) => {
    ...
    for (const label of windows) {
      ...
      out.push(
        `### New in the ${label}`,
        "",
        ...renderTable({ rows: inWindow, list: "recs", sort: "score", linkPrefix: "views/", showWhy: true, repo, survey }),
        ""
      );
    }
    ...
    if (rest.length) {
      ...
      if (rest.length > preview) {
        out.push("", `[... and ${rest.length - preview} more, sorted by score](${viewFile("recs", "score")})`);
      }
    }
    return out;
  };
