import { log } from "./log.js";

const BASE = "https://api.semanticscholar.org/graph/v1/paper";
const AUTHOR_BASE = "https://api.semanticscholar.org/graph/v1/author";

// The metadata shown in the survey table. Deliberately excludes `abstract`:
// it roughly triples the size of the committed JSON and `tldr` already gives
// us a one-line summary.
const CORE_FIELDS = [
  "paperId",
  "externalIds",
  "url",
  "title",
  "tldr",
  "venue",
  "year",
  "publicationDate",
  // Nested author fields cost nothing: they ride along in the same batch
  // request. An h-index and an affiliation are the only quality signals that
  // mean anything for a paper published this month, since every
  // citation-based measure is still zero for it.
  "authors.name",
  "authors.hIndex",
  "authors.affiliations",
  "citationCount",
  "referenceCount",
  "influentialCitationCount",
  "openAccessPdf",
].join(",");

const CORE_CHUNK = 200;

// Citations are fetched one paper at a time rather than in a batch. A batch
// response is capped at roughly 9,999 citations in total, so a single hugely
// cited paper (Attention Is All You Need has over 100,000) swallows the entire
// budget and every other paper silently returns none -- which collapses the
// overlap ranking to zero. Per-paper requests give each paper its own cap.
//
// Capping at 1,000 is also better signal, not just cheaper: papers cited by
// everything say little about what makes *this* survey's topic cohere.
const CITATIONS_PER_PAPER = 1000;

// The shared unauthenticated pool can throttle for minutes at a time, so we are
// patient: eight attempts with the wait capped at a minute is roughly five
// minutes of trying before giving up and letting the next run retry.
const MAX_ATTEMPTS = 8;
const MAX_BACKOFF_MS = 60_000;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

// How many graph requests to have in flight at once. The citation and
// reference passes are one request per paper, so a 474-paper survey is ~950
// round trips; sequentially that is a very long run. Four is deliberately
// modest: the unauthenticated pool is shared with everyone and throttles under
// load, so pushing this higher mostly buys more 429s and more backoff.
export const DEFAULT_CONCURRENCY = 4;

/**
 * Runs `fn` over `items` with at most `limit` in flight, preserving order.
 * Reports progress, since these passes are the slow part of a run and silence
 * for several minutes looks like a hang.
 */
const mapPool = async (items, limit, fn, label) => {
  const results = new Array(items.length);
  let next = 0;
  let done = 0;
  const started = Date.now();

  const worker = async () => {
    while (next < items.length) {
      const i = next++;
      results[i] = await fn(items[i], i);
      done++;
      if (label && (done % 25 === 0 || done === items.length)) {
        const rate = done / ((Date.now() - started) / 1000);
        const left = Math.round((items.length - done) / Math.max(rate, 0.01));
        log.info(
          `${label}: ${done}/${items.length}` +
            (done < items.length ? `, about ${left}s left` : "")
        );
      }
    }
  };

  await Promise.all(Array.from({ length: Math.min(limit, items.length) }, worker));
  return results;
};

// Jitter stops several requests from retrying in lockstep.
const backoff = (wait) => Math.min(wait, MAX_BACKOFF_MS) * (0.75 + Math.random() * 0.5);

const chunk = (arr, size) => {
  const out = [];
  for (let i = 0; i < arr.length; i += size) out.push(arr.slice(i, i + size));
  return out;
};

/**
 * POSTs one batch of ids, retrying on throttling and transient server errors.
 *
 * The API's shared unauthenticated pool returns 429 readily (we saw three in a
 * row during development), so backoff here is load-bearing rather than
 * defensive. Crucially a 429 is never reported as "paper not found" -- the
 * original project conflated the two, which made throttling look like missing
 * papers.
 */
const postBatch = async (ids, fields) => {
  let wait = 2000;

  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    let res;
    try {
      res = await fetch(`${BASE}/batch?fields=${fields}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ids }),
      });
    } catch (err) {
      log.warn(`Network error talking to Semantic Scholar (attempt ${attempt}): ${err.message}`);
      await sleep(backoff(wait));
      wait *= 2;
      continue;
    }

    if (res.ok) {
      const data = await res.json();
      if (Array.isArray(data)) return data;
      // The endpoint sometimes answers 200 with an error object/string.
      log.warn(`Unexpected batch response shape: ${JSON.stringify(data).slice(0, 200)}`);
      return ids.map(() => null);
    }

    if (res.status === 429 || res.status >= 500) {
      const retryAfter = Number(res.headers.get("retry-after"));
      const delay = Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter * 1000 : backoff(wait);
      log.warn(
        `Semantic Scholar returned ${res.status} (attempt ${attempt}/${MAX_ATTEMPTS}); waiting ${Math.round(delay / 1000)}s. This is throttling, not a missing paper.`
      );
      await sleep(delay);
      wait *= 2;
      continue;
    }

    const body = await res.text().catch(() => "");
    log.warn(`Semantic Scholar returned ${res.status}: ${body.slice(0, 200)}`);
    return ids.map(() => null);
  }

  log.error(
    `Gave up on a batch of ${ids.length} papers after ${MAX_ATTEMPTS} attempts. They will be retried on the next run.`
  );
  return ids.map(() => null);
};

const normaliseAuthors = (authors) =>
  (authors ?? []).map((a) => a?.name).filter(Boolean);

/**
 * The two best-established authors on a paper, by h-index.
 *
 * Best rather than first, and two rather than all: a long author list is
 * mostly noise, the first author is usually the most junior, and the senior
 * name is the one that tells you whether this is likely to be worth reading.
 * Mean and sum are both wrong here -- a mean is dragged down by every student
 * on the paper, a sum just rewards long author lists.
 */
export const topAuthorsOf = (authors) =>
  (authors ?? [])
    .filter((a) => a?.name && Number.isFinite(a?.hIndex))
    .sort((a, b) => b.hIndex - a.hIndex)
    .slice(0, 2)
    // `authorId` comes back whether or not it is asked for, and is what makes
    // the name a link to that author's page.
    .map((a) => ({ name: a.name, hIndex: a.hIndex, authorId: a.authorId ?? null }));

/**
 * Where a paper comes from: the one or two affiliations most of its authors
 * share.
 *
 * Semantic Scholar has no notion of an institution as an entity -- these are
 * free-text strings on an author, present maybe half the time -- so "most
 * popular" is a count across this paper's own authors, broken by the h-index
 * of whoever carries it. That last part matters: with one author each at two
 * places, the established one is the more informative answer.
 */
export const affiliationsOf = (authors) => {
  const weight = new Map();
  const spare = [];

  for (const author of authors ?? []) {
    const listed = (author?.affiliations ?? []).map((a) => String(a).trim()).filter(Boolean);
    const h = Number.isFinite(author?.hIndex) ? author.hIndex : -1;

    // Only an author's first affiliation votes. Counting all of them let one
    // well-affiliated author fill both slots with their own two employers --
    // "UC Irvine" then "Envive AI (fka Spiffy AI)" -- when the informative
    // answer is the second *lab*, from a different author.
    const [primary, ...rest] = listed;
    if (primary) {
      const seen = weight.get(primary) ?? { count: 0, best: -1 };
      weight.set(primary, { count: seen.count + 1, best: Math.max(seen.best, h) });
    }
    for (const other of rest) spare.push([other, h]);
  }

  const ranked = [...weight.entries()]
    .sort((a, b) => b[1].count - a[1].count || b[1].best - a[1].best)
    .map(([name]) => name);

  // Only if the primaries do not fill both slots does a second affiliation
  // get to stand in, so a single-author paper still says something.
  if (ranked.length < 2) {
    for (const [name] of spare.sort((a, b) => b[1] - a[1])) {
      if (!ranked.includes(name)) ranked.push(name);
      if (ranked.length === 2) break;
    }
  }

  return ranked.slice(0, 2);
};

/** Reshapes an API record into the flat form we store in data/papers.json. */
const toRecord = (raw, sourceLink) => {
  if (!raw?.paperId) return null;
  const ext = raw.externalIds ?? {};
  return {
    id: raw.paperId,
    title: raw.title ?? "Untitled",
    authors: normaliseAuthors(raw.authors),
    topAuthors: topAuthorsOf(raw.authors),
    affiliations: affiliationsOf(raw.authors),
    year: raw.year ?? null,
    venue: raw.venue || null,
    publicationDate: raw.publicationDate ?? null,
    summary: raw.tldr?.text ?? "",
    url: raw.url ?? `https://www.semanticscholar.org/paper/${raw.paperId}`,
    doi: ext.DOI ?? null,
    arxivId: ext.ArXiv ?? null,
    pdf: raw.openAccessPdf?.url || null,
    citationCount: raw.citationCount ?? 0,
    referenceCount: raw.referenceCount ?? 0,
    influentialCitationCount: raw.influentialCitationCount ?? 0,
    source: sourceLink ?? null,
    addedAt: new Date().toISOString().slice(0, 10),
  };
};

/**
 * Looks up many papers. `requests` is [{ id, source }].
 * Returns { found: Record[], missing: [{id, source}] }.
 */
export const fetchPapers = async (requests) => {
  const found = [];
  const missing = [];
  if (!requests.length) return { found, missing };

  const batches = chunk(requests, CORE_CHUNK);
  log.info(
    `Looking up ${requests.length} paper(s) from Semantic Scholar in ${batches.length} batch request(s).`
  );

  for (const [i, batch] of batches.entries()) {
    log.debug(`Core batch ${i + 1}/${batches.length} (${batch.length} ids)`);
    const results = await postBatch(
      batch.map((r) => r.id),
      CORE_FIELDS
    );

    results.forEach((raw, idx) => {
      const req = batch[idx];
      const record = toRecord(raw, req.source);
      if (record) found.push(record);
      else missing.push(req);
    });
  }

  log.info(`Semantic Scholar resolved ${found.length}, missed ${missing.length}.`);
  return { found, missing };
};

/**
 * Fetches, for each given paper, the ids of the papers it cites.
 *
 * Also returns how often each cited paper is cited globally, which the
 * backward ranking needs to discount ubiquitous works.
 */
export const fetchReferenceIdsFor = async (paperIds, concurrency = DEFAULT_CONCURRENCY) => {
  const edges = new Map();
  const citationCounts = new Map();
  if (!paperIds.length) return { edges, citationCounts };

  log.info(
    `Fetching reference lists for ${paperIds.length} paper(s), ${concurrency} at a time.`
  );

  await mapPool(paperIds, concurrency, async (paperId) => {
    const url = `${BASE}/${paperId}/references?fields=paperId,citationCount,intents,isInfluential&limit=1000`;
    let wait = 2000;
    let rows = null;

    for (let attempt = 1; attempt <= MAX_ATTEMPTS && rows === null; attempt++) {
      try {
        const res = await fetch(url);
        if (res.ok) {
          rows = (await res.json())?.data ?? [];
        } else if (res.status === 429 || res.status >= 500) {
          const retryAfter = Number(res.headers.get("retry-after"));
          await sleep(
            Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter * 1000 : backoff(wait)
          );
          wait *= 2;
        } else {
          log.warn(`Could not fetch references for ${paperId}: HTTP ${res.status}.`);
          rows = [];
        }
      } catch (err) {
        log.debug(`Reference fetch error (attempt ${attempt}): ${err.message}`);
        await sleep(backoff(wait));
        wait *= 2;
      }
    }

    // Every other fetcher says so when it runs out of attempts. This one
    // returned empty edges in silence, which looks exactly like a paper that
    // cites nothing and quietly weakens both the ranking and the Score.
    if (rows === null) {
      log.warn(`Gave up fetching references for ${paperId} after ${MAX_ATTEMPTS} attempts.`);
    }

    const ids = [];
    const strong = [];
    for (const row of rows ?? []) {
      const cited = row?.citedPaper;
      if (!cited?.paperId) continue;
      ids.push(cited.paperId);
      // The intent sits on the edge, not on either paper, so it has to be
      // read here while both ends are in hand.
      if (isStrongEdge(row)) strong.push(cited.paperId);
      citationCounts.set(cited.paperId, cited.citationCount ?? 0);
    }
    edges.set(paperId, { ids, strong });
  }, "references");

  const total = [...edges.values()].reduce((n, edge) => n + edge.ids.length, 0);
  log.info(`Collected ${total} reference edges.`);
  return { edges, citationCounts };
};

/**
 * Is this citation more than a mention?
 *
 * Semantic Scholar labels an edge `background`, `methodology` or `result`, and
 * separately flags a few as influential. Background is the field's plumbing --
 * the sentence in the introduction that cites the famous benchmark -- while
 * methodology and result mean the citing work actually used the thing.
 *
 * Measured on real edges: about 59% carry any intent at all and 12% are
 * influential, so this can only ever be a bonus. An edge with no labels is
 * treated exactly as it was before, never penalised for the API's silence.
 */
export const isStrongEdge = (row) => {
  if (row?.isInfluential) return true;
  const intents = row?.intents ?? [];
  return intents.includes("methodology") || intents.includes("result");
};

/** GETs one paper's citation list, with the same backoff policy as batches. */
const getCitations = async (paperId) => {
  const url = `${BASE}/${paperId}/citations?fields=paperId,intents,isInfluential&limit=${CITATIONS_PER_PAPER}`;
  let wait = 2000;

  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    let res;
    try {
      res = await fetch(url);
    } catch (err) {
      log.warn(`Network error fetching citations (attempt ${attempt}): ${err.message}`);
      await sleep(backoff(wait));
      wait *= 2;
      continue;
    }

    if (res.ok) {
      const data = await res.json();
      const rows = (data?.data ?? []).filter((d) => d?.citingPaper?.paperId);
      return {
        ids: rows.map((d) => d.citingPaper.paperId),
        strong: rows.filter(isStrongEdge).map((d) => d.citingPaper.paperId),
      };
    }

    if (res.status === 429 || res.status >= 500) {
      const retryAfter = Number(res.headers.get("retry-after"));
      const delay = Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter * 1000 : backoff(wait);
      log.debug(`Citations for ${paperId}: ${res.status}, waiting ${Math.round(delay / 1000)}s.`);
      await sleep(delay);
      wait *= 2;
      continue;
    }

    log.warn(`Could not fetch citations for ${paperId}: HTTP ${res.status}.`);
    return { ids: [], strong: [] };
  }

  log.warn(`Gave up fetching citations for ${paperId} after ${MAX_ATTEMPTS} attempts.`);
  return { ids: [], strong: [] };
};

/**
 * Fetches the papers a given paper cites.
 *
 * This is what makes "seed from a survey" work: a survey's reference list is a
 * reading list someone already curated by hand, so one request turns a single
 * link into a whole starting bibliography.
 */
export const fetchReferences = async (identifier) => {
  const url = `${BASE}/${encodeURIComponent(identifier)}/references?fields=paperId,title&limit=1000`;
  let wait = 2000;

  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    let res;
    try {
      res = await fetch(url);
    } catch (err) {
      log.warn(`Network error fetching references (attempt ${attempt}): ${err.message}`);
      await sleep(backoff(wait));
      wait *= 2;
      continue;
    }

    if (res.ok) {
      const data = await res.json();
      const ids = (data?.data ?? []).map((d) => d?.citedPaper?.paperId).filter(Boolean);
      log.info(`${identifier} cites ${ids.length} paper(s).`);
      return ids;
    }
    if (res.status === 404) {
      log.warn(`Could not find ${identifier}, so its references could not be used as seeds.`);
      return [];
    }
    if (res.status === 429 || res.status >= 500) {
      const retryAfter = Number(res.headers.get("retry-after"));
      const delay = Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter * 1000 : backoff(wait);
      log.warn(`Semantic Scholar returned ${res.status} fetching references; waiting.`);
      await sleep(delay);
      wait *= 2;
      continue;
    }
    log.warn(`Could not fetch references for ${identifier}: HTTP ${res.status}.`);
    return [];
  }
  log.warn(`Gave up fetching references for ${identifier}.`);
  return [];
};

/**
 * A GET with the same patience as the rest of this file: retry on throttling
 * and transient server errors, treat 404 as a real answer, and say so when we
 * run out of attempts rather than returning an empty result in silence.
 */
const getJson = async (url, what) => {
  let wait = 2000;

  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    let res;
    try {
      res = await fetch(url);
    } catch (err) {
      log.warn(`Network error fetching ${what} (attempt ${attempt}): ${err.message}`);
      await sleep(backoff(wait));
      wait *= 2;
      continue;
    }

    if (res.ok) return await res.json();
    if (res.status === 404) {
      log.warn(`Semantic Scholar does not know ${what}.`);
      return null;
    }
    if (res.status === 429 || res.status >= 500) {
      const retryAfter = Number(res.headers.get("retry-after"));
      const delay = Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter * 1000 : backoff(wait);
      log.warn(
        `Semantic Scholar returned ${res.status} fetching ${what} (attempt ${attempt}/${MAX_ATTEMPTS}); waiting ${Math.round(delay / 1000)}s.`
      );
      await sleep(delay);
      wait *= 2;
      continue;
    }
    log.warn(`Could not fetch ${what}: HTTP ${res.status}.`);
    return null;
  }

  log.warn(`Gave up fetching ${what} after ${MAX_ATTEMPTS} attempts; the next run will retry.`);
  return null;
};

// One page of an author's publication list. The endpoint allows 1000, and
// nobody's bibliography is long enough for this to matter twice.
const AUTHOR_PAGE = 1000;

/**
 * Every paper Semantic Scholar attributes to one author profile.
 *
 * Returns `null` rather than an empty list when the lookup failed, because the
 * caller must not treat "throttled" as "this author has published nothing" and
 * write an empty survey.
 */
export const fetchAuthorPapers = async (authorId) => {
  const fields = "paperId,title,year";
  const papers = [];
  let offset = 0;

  for (let page = 0; page < 10; page++) {
    const url = `${AUTHOR_BASE}/${encodeURIComponent(authorId)}/papers?fields=${fields}&limit=${AUTHOR_PAGE}&offset=${offset}`;
    const data = await getJson(url, `the publication list for author ${authorId}`);
    if (!data) return papers.length ? papers : null;

    for (const row of data.data ?? []) {
      if (row?.paperId) papers.push({ id: row.paperId, title: row.title ?? null, year: row.year ?? null });
    }
    if (!data.next) break;
    offset = data.next;
  }

  log.info(`Semantic Scholar lists ${papers.length} paper(s) for author ${authorId}.`);
  return papers;
};

/**
 * The display name on one author profile.
 *
 * Needed because the publication list does not carry it, so a survey seeded
 * from a profile URL would otherwise have nothing to call the person but the
 * URL itself -- which is what it then puts in its logs and its front page.
 */
export const fetchAuthorName = async (authorId) => {
  const data = await getJson(`${AUTHOR_BASE}/${encodeURIComponent(authorId)}?fields=name`, `author ${authorId}`);
  return data?.name ?? null;
};

/**
 * Finds author profiles by name, most published first.
 *
 * Names are ambiguous and Semantic Scholar splits one person across profiles
 * more often than it merges two people into one, so the caller shows what it
 * picked and what it passed over rather than deciding silently.
 */
export const searchAuthors = async (name) => {
  const url = `${AUTHOR_BASE}/search?query=${encodeURIComponent(name)}&fields=name,paperCount,hIndex&limit=10`;
  const data = await getJson(url, `authors matching "${name}"`);
  // Ordering is the caller's business: see resolveAuthorPapers, which has to
  // both pick a profile and explain the ones it passed over.
  return (data?.data ?? [])
    .filter((a) => a?.authorId)
    .map((a) => ({
      authorId: a.authorId,
      name: a.name ?? name,
      paperCount: a.paperCount ?? 0,
      hIndex: a.hIndex ?? 0,
    }));
};

/**
 * Resolves a bare title to a paper. Used for bibliography entries that carry
 * no DOI, arXiv id or URL, which is common in hand-maintained .bib files.
 */
export const matchByTitle = async (title) => {
  const url = `${BASE}/search/match?query=${encodeURIComponent(title)}&fields=paperId,title`;
  try {
    const res = await fetch(url);
    if (!res.ok) return null;
    const hit = (await res.json())?.data?.[0];
    if (!hit?.paperId) return null;
    log.debug(`Title matched: "${title.slice(0, 50)}" -> ${hit.title?.slice(0, 50)}`);
    return hit.paperId;
  } catch (err) {
    log.debug(`Title match failed for "${title.slice(0, 40)}": ${err.message}`);
    return null;
  }
};

/**
 * Fetches the ids of papers citing each given paper, used to rank candidates.
 * Returns Map<paperId, { ids, strong }>, where `strong` is the subset the API
 * called influential or labelled as methodology or result.
 */
export const fetchCitationIds = async (paperIds, concurrency = DEFAULT_CONCURRENCY) => {
  const out = new Map();
  if (!paperIds.length) return out;

  log.info(`Fetching citation lists for ${paperIds.length} paper(s), ${concurrency} at a time.`);

  await mapPool(
    paperIds,
    concurrency,
    async (paperId) => {
      out.set(paperId, await getCitations(paperId));
    },
    "citations"
  );

  const counts = [...out.values()].map((edge) => edge.ids.length);
  const total = counts.reduce((n, c) => n + c, 0);
  const strong = [...out.values()].reduce((n, edge) => n + edge.strong.length, 0);
  if (total) log.stat("citations that build on the work", `${strong} of ${total}`);
  const empty = counts.filter((c) => c === 0).length;
  log.info(`Collected ${total} citation edges across ${paperIds.length} paper(s).`);
  if (empty) {
    log.warn(
      `${empty} paper(s) returned no citations. If that seems wrong it is usually throttling; the next run will retry.`
    );
  }
  return out;
};
