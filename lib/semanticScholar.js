 ...
 */
export const fetchReferenceIdsFor = async (paperIds) => {
  const edges = new Map();
  const citationCounts = new Map();
  if (!paperIds.length) return { edges, citationCounts };

  log.info(`Fetching reference lists for ${paperIds.length} paper(s), one request each.`);
  ...
  for (const [i, paperId] of paperIds.entries()) {
    ...
    log.debug(`(${i + 1}/${paperIds.length}) ${paperId} cites ${ids.length}`);
  }
