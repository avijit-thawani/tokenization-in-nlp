...
export const resolveAll = (lines) => {
  ...
  for (const line of lines) {
    ...
    // "author: <profile>" means: everything this person has published is Core.
    // Parsing the profile itself lives in lib/authors.js; this only has to
    // spot the prefix and hand the rest over.
    const authorMatch = trimmed.match(/^authors?\s*:\s*(.+)$/i);
    if (authorMatch) {
      authors.push({ target: authorMatch[1].trim(), source: trimmed });
      continue;
    }
  ...
  return { resolved, unresolved, seedFrom, authors, titles };
};
