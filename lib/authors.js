...
import { fetchAuthorPapers, searchAuthors } from "./semanticScholar.js";
 ...
 */
export const resolveAuthorPapers = async ({ target, email, search = searchAuthors, papersFor = fetchAuthorPapers }) => {
  ...
  // `name` is null when the line named a Semantic Scholar id directly, since
  // the publication list does not carry the author's name. Returned as null
  // rather than as "author 2090730520", so a caller holding a real name from
  // an earlier run keeps it instead of overwriting it with the id.
