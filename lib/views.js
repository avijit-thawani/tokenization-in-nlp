 ...
 */
export const renderTable = ({ rows, list, sort, linkPrefix = "", showWhy = false, startIndex = 1, repo = "" }) => {
  if (!rows.length) {
    return list === "core"
  ...
  rows.forEach((p, i) => {
    ...
    lines.push(showWhy ? `${main} ${cell(p.why)} | ${decideCell(p, repo)} |` : main);
  });
  return lines;
};

/** A standalone `views/<list>-by-<sort>.md` file. */
export const renderView = ({ list, sort, rows, config, updated, repo = "" }) => {
  ...
  return [
    ...
    "",
    ...renderTable({ rows, list, sort, showWhy: list === "recs", repo }),
    "",
    "---",
    "",
...
export const allViews = ({ core, recs, config, updated, repo = "" }) => {
  const files = [];
  for (const [list, rows] of [
    ...
    for (const sort of Object.keys(SORTS)) {
      ...
      files.push({
        ...
        body: `${renderView({ list, sort, rows: sorted, config, updated, repo })}\n`,
      });
    }
  }
