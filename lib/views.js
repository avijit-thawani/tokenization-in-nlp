...
const decideCell = (rec, repo, survey = "") => {
  ...
  // A row whose `add` goes to a pull request says so. It used to look
  // identical to a row whose `add` opens an issue, so clicking two of them in
  // a row landed you in two different places with no warning.
  const add = rec.prUrl
    ? `[add via PR](${rec.prUrl})`
    : `[add](https://github.com/${repo}/issues/new?labels=add-paper` +
