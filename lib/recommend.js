...
export const buildRecommendations = async ({
   ...
   */
  const specificity = (edgeMap, penalty) => {
    const counts = new Map();
    ...
    for (const [seedId, ids] of edgeMap) {
      ...
      for (const id of ids) {
        if (!id || inSurvey.has(id) || dismissed.has(id)) continue;
        counts.set(id, (counts.get(id) ?? 0) + 1);
        weights.set(id, (weights.get(id) ?? 0) + worth);
      }
    }
  ...
  const { citationEdges, referenceEdges, citationCounts } = await log.phase("refreshing the citation graph", () =>
    refreshGraph({
      graph,
      paperIds: seedIds,
  ...
  // of your list counted the same.
  const forwardSpecificity = specificity(citationEdges, Number(algorithm.specificityPenalty ?? 0.3));
