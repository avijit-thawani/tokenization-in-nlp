 ...
 */
export const resolveIdentity = (config, event, repoFullName = process.env.GITHUB_REPOSITORY) => {
  ...
  //
  // Passed in rather than read here, so a test can say which of those it is
  // testing. Reading the environment directly made this function's result
  // depend on where it ran: every case passed locally and the "no repository
  // at all" case failed in CI, where GITHUB_REPOSITORY is always set.
  const envName = String(repoFullName ?? "").split("/")[1] || null;
